...
require 'config.php';
//Is Submit is Success Move to UserHome Page
...
if(isset($_POST["submit"])){
  ...
  $result = mysqli_query($conn, "SELECT * FROM users WHERE username = '$username'"); //responsible for establishing a connection to the database
  $row = mysqli_fetch_assoc($result); //responsible for establishing a connection to the database
  if(mysqli_num_rows($result) > 0){
    if($password == $row['password']){
      $_SESSION["login"] = true; //responsible for establishing a connection to the database
      $_SESSION["id"] = $row["id"]; // stores the user's ID
      header("Location: Userhome.php");
    }
    else{
      echo
      "<script> alert('Wrong Password'); </script>"; //Password Inncorrect send alert
    }
  }
  else{
    echo
    "<script> alert('User Not Registered'); </script>"; //User name do not Fetch then send alert User Not Registered
  }
}
?>
